...
func (node *ConcatExpr) Format(buf *TrackedBuffer) {
	buf.Fprintf("%s || %s", String(node.Left), String(node.Right))
}
...
type UnknowStmt struct {
	String string
}

func (*UnknowStmt) IStatement() {}

func (node *UnknowStmt) Format(buf *TrackedBuffer) {
	buf.Fprintf("%s", node.String)
}
