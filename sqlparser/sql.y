...
%token <empty> ALL DISTINCT AS EXISTS NULL ASC DESC VALUES INTO DUPLICATE KEY DEFAULT SET SHOW LOCK
%token <bytes> ID STRING NUMBER VALUE_ARG COMMENT
%token <empty> '(' '~'
%token <empty> TIME ZONE SESSION AUTHORIZATION
...
// Transaction Tokens
%token <empty> BEGIN START TRANSACTION COMMIT ROLLBACK ISOLATION LEVEL
...
%type <statement> insert_statement update_statement delete_statement set_statement show_statement
